...
import { ColDef, ColGroupDef, GridApi, ProcessCellForExportParams } from 'ag-grid-community';
...
import { CONFIGITEM__Power_Rails_Columns, PermissionActionEnum, PowerInfoAspectEnum, SPECIAL_RED_COLOR, UIMessageType } from "../../DataModels/Constants";
...
import { getDateAppendedName, getHumanReadableByteSize } from "../../BizLogicUtilities/UtilFunctions";
import { isUserApprovedForCoreAction } from "../../BizLogicUtilities/Permissions";
import { SpButton } from "../../CommonComponents/SimplePieces";



interface PowerRailsTabProps {

}

const PowerRailsTab: React.FC<PowerRailsTabProps> = ({ }) => {
    ...
    const [powerRails, setPowerRails] = useState<BasicProperty[]>([])
    ...
    const columnConfigs = useMemo(() => {
        let configColProps : PropertyItem[] = initConfigs?.filter(a => a.configName === CONFIGITEM__Power_Rails_Columns)?.at(0)?.configValue ?? undefined
    ...
    const columnDefinitions = useMemo(() => {
        const columnDefs: Array<ColDef | ColGroupDef> = [
            ...
            },
            {
                ...
                cellRenderer: function (params: any) {
                    return (
                        <Tooltip key={`pw-tt-rem-${params.data.id}`} placement="top" title={`Delete power rails entry`}>
                            <span>
        ...
        if(columnConfigs && columnConfigs.length > 0) {
            let filterList = columnConfigs.filter((a: PropertyItem) => a.enabled === true && a.category.toUpperCase() === PowerInfoAspectEnum.RAILS.toString().toUpperCase())
    ...
    function onRemovalAction(event: any, item: BasicProperty): void {
        if (isUserApprovedForCoreAction(loggedInUser, project, PermissionActionEnum.UPDATE_POWER_RAILS) === false) { return; }
        if(item && item.id && item.id.length > 0) {
            let newSet = powerRails.filter(a => a.id !== item.id)
            setPowerRails([...newSet]);
        }
    }
        ...
        if(gridApi) {
            if(columnConfigs && columnConfigs.length > 0) {
                let newEntry : BasicProperty = {
                    ...
                    name: powerRails.length.toString(),
                ...
                let newArr = powerRails.concat([newEntry])
                setPowerRails([...newArr]);
    ...
    function handleSavePowerAspect(): void {
        ...
        let saveConfirmData: ConfirmationDialogProps = {
            ...
            warningText_main: (powerRails.length > 0)
                ? `Are you sure you want to save power info for current project?`
                : `No power rails in scope. Are you sure you want to clear any/all power rails entries for the current project?`,
            warningText_other: "",
        ...
        if(files && files.length > 0) {
            ...
            if(powerRails && powerRails.length > 0) {
                let delConfirmData: ConfirmationDialogProps = {
                    onFormClosed: onConfirmationDataAvailable,
                    title: "Please Confirm",
                    warningText_main: `Are you sure you want to import fresh Power rails info?`,
                    warningText_other: "Warning! All existing Rails data will be ovewritten!",
                    actionButtonText: "Proceed",
                    enableSecondaryActionButton: false,
                    secondaryActionButtonText: "",
            ...
            if(contextualInfo.key === "SAVE_ACTION") {
                if(action === ConfirmationDialogActionType.PROCEED) {
                    ...
                    if(powerInfo && powerInfo._id) {
                        ...
                        powerInfoToSave.rails = [...powerRails]
                    }
                    else {
                        let newPI: PowerInfo = {
                            _id: "",
                    ...
                    replacePowerInfo(powerInfoToSave, true, false).then((updatedPowerInfo: PowerInfo) => {
                        if(updatedPowerInfo && updatedPowerInfo._id) {
                            setPowerRails(updatedPowerInfo.rails);
                            displayQuickMessage(UIMessageType.SUCCESS_MSG, `Power rails data update completed!`)
                        }
                    })                
                }
            }
                ...
                if(action === ConfirmationDialogActionType.PROCEED) {
                    ...
                    if(files && files.length > 0) {
                        let resultPowerInfo : PowerInfo = await uploadPowerInfo(files[0], project?._id?.toString() as string, PowerInfoAspectEnum.RAILS)
                        if(resultPowerInfo) {
                            setPowerRails(resultPowerInfo.rails)
                            displayQuickMessage(UIMessageType.SUCCESS_MSG, "Power rails upload process completed")
                        }
                    }
                }
            }
...
export default PowerRailsTab
    ...
    // function getFormattedValueForGrid(params: ProcessCellForExportParams<any, any>): string {
    ...
    //     fileName : getDateAppendedName(project.name.toUpperCase().replaceAll(" ", "_") + "__PowerRails"),
    //     columnSeparator: ',',
    //     processCellCallback: (params) => getFormattedValueForGrid(params)
    // }}