...
import { ProjectPropertyCategoryEnum, SPECIAL_QUARTZ_COLOR, SPECIAL_RED_COLOR, UIMessageType } from "../../DataModels/Constants";
...
import { SPDomainData, User } from "../../DataModels/HelperModels";
...
const ProjectPermissionsTab: React.FC<ProjectPermissionsTabProps> = ({  }) => {
    ...
    useEffect(() => {
        ...
        if(permissionRoles && permissionRoles.length > 0) {
            setLoadingSpinnerCtx({enabled: true, text: "Retrieving roles and permission assignments for current project. Please wait..."} as LoadingSpinnerInfo)
            permissionRoles?.forEach(x => roleIdToRoleNameMapping.current.set(x.id, x.name))  //Important!
            getPreloadPermissions(project._id?.toString() as string, permissionRoles).then((usersPerRoleMapping: Map<string, User[]>) => {
    ...
    async function performPermissionsUpdate(): Promise<void> {
        ...
        let permActionResult : QuickStatus<any> = await updateProjectPermissions(loggedInUser as LoggedInUser, project, roleUsers).finally(() => { cancelLoadingSpinnerCtx() })
