...
import { checkDuplicatesIgnoreCase, containsSpecialChars, getFilterForBucket } from "../../BizLogic/generalUtils";
import { performBucketAdd, performBucketDelete, performBucketExport, performConfigAdd, processCSVContent } from "../../BizLogic/midwareUtils";
...
bucketRouter.get("/:env/buckets/for-app", async (req: Request, res: Response) => {
    try {
        ...
        let existingAppBuckets = (await collection.find(appBucketFilter).toArray()) as Bucket[];
        res.status(200).send({ payload: existingAppBuckets } as ResponseData);
    } 
    catch (e: any) {
        let resp = {
            payload: undefined,
